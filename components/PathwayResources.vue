  ...
  for (const step of store.pathway.steps) {
    for (const activity of step.activities) {
      if (activity.videoUrls && activity.videoUrls.length > 0) {
        // Check if we have the enhanced YouTube metadata
        if (
          (activity as any).youtubeVideos &&
          (activity as any).youtubeVideos.length > 0
        ) {
          // Use the enhanced metadata
          const videoMetadata = (activity as any).youtubeVideos[0];
          videos.push({
            title: videoMetadata.title || activity.name,
            url: videoMetadata.url,
            description: videoMetadata.description || activity.description,
          });
        } else {
          // Fall back to basic info
          videos.push({
            title: activity.name,
            url: activity.videoUrls[0],
